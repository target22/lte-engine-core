...
1. REF-GATED PAYLOAD LOCK (config: linter_rules.json#payload_lock).
   A contract node with at least one inbound [!ref-...] in the same locale
   is locked: its body must stay byte-identical and its anchor must keep
   existing. In a document holding a locked node, front matter keys outside
...
2. ATOMIC DEPRECATION GUARDRAIL (config: linter_rules.json#dependent_lifecycle).
   ...
   file is deleted, drops the [!ref-...], or is itself moved to an
...
def parse_image(grammar: Grammar, kinds: Mapping, rel_path: str, text: str) -> tuple:
    return ast_blocks.parse_text(grammar, kinds, rel_path, text)


def reference_map(grammar: Grammar, kinds: Mapping, images: Mapping, *, layout) -> dict:
    ...
    for rel_path in sorted(images):
        ...
        if text is None or locale is None:
            continue
        _, callouts = parse_image(grammar, kinds, rel_path, text)
        for callout in callouts:
            refs.setdefault((locale, callout.target_id), set()).add(rel_path)
    return refs


def node_bodies(grammar: Grammar, kinds: Mapping, rel_path: str, text: str) -> dict:
    nodes, _ = parse_image(grammar, kinds, rel_path, text)
...
def check_payload_lock(grammar: Grammar, kinds: Mapping, change: Change,
    ...
    if head_text is None or locale is None:
        return []
    head_nodes = node_bodies(grammar, kinds, change.old_path, head_text)
    ...
    violations = []
    index_nodes = node_bodies(grammar, kinds, change.path, index_text)
...
def effective_statuses(grammar: Grammar, kinds: Mapping, rel_path: str, text: str) -> dict:
...
def newly_invalidating(grammar: Grammar, kinds: Mapping, changes: Sequence,
    ...
    for change in changes:
        ...
        if head_text is None or index_text is None:
            continue
        before = effective_statuses(grammar, kinds, change.old_path, head_text)
        after = effective_statuses(grammar, kinds, change.path, index_text)
...
def resolution_of(grammar: Grammar, kinds: Mapping, anchor: str, dependent_path: str,
    ...
    _, callouts = parse_image(grammar, kinds, change.path, text)
...
def check_atomic_deprecation(grammar: Grammar, kinds: Mapping, transitions: Mapping,
    ...
    for (locale, anchor) in sorted(transitions, key=lambda k: (k[0] or "", k[1])):
        ...
        for dependent in dependents:
            reason = resolution_of(grammar, kinds, anchor, dependent,
